    ...
    def play(self):
        ...
        while True:
            ...
            if (self.y_bird > self.ground+4) or (self.y_bird < self.roof-4):
                ...
                self.y_increase = 0
                self.pipes_stop = 1
            elif ((self.x_bird + self.img_sizes['Bird'][0])>=self.x_pipe+3)and(self.x_bird<=(self.x_pipe+50)):
                if ((self.y_bird + self.img_sizes['Bird'][1])>=self.y_pipe)or(self.y_bird<=(self.y_pipe-self.space_between_pipes)):
                    # Bird dies
                    self.gameover()
                    pygame.display.flip()
                    return 2
                    self.y_increase = 0
                    self.pipes_stop = 1
            elif ((self.x_bird + self.img_sizes['Bird'][0])>=self.x_second_pipe+3)and(self.x_bird<=(self.x_second_pipe+50)):
                if ((self.y_bird + self.img_sizes['Bird'][1])>=self.y_second_pipe)or(self.y_bird<=(self.y_second_pipe-self.space_between_pipes)):
                    # Bird dies
                    self.gameover()
                    pygame.display.flip()
                    return 2
